        ...
        <div class="shareMain" v-for="item in list" v-show="list.length>0">
            <div class="shareList">
                <div style="padding: 0 .24rem;">
                    <div class="shareTop">
                        <div class="shareTopLift"><img :src="user.avatar" alt=""></div>
                        <div class="shareTopRight">
                            <p>{{user.name}}<span style="margin-left: .1rem;" v-show="user.position !== ''">{{user.position}}</span></p>
                            <p>{{item.createTime}}
                                <span v-if="item.status == 0">(未审核)</span>
                                <span v-else-if="item.status == -1">(审核未通过)</span>
                                <span v-else>(审核已通过)</span>
                            </p>
                        </div>
                    </div>
            ...
            <div id="annexMerge">
                ...
                <div class="comment bottomDiv" style="padding-bottom: 0; display: none;">
                    <div class="lists" style="margin-top:0;" >
                        ...
                        <router-link :to="{path: '/shareDetail', query: { id: item.id }}" class="moreList" v-show="item.comments>4">查看更多评论</router-link>
                    </div>
                </div>
                <!--点赞-->
    ...
    export default {
        ...
        methods:{
            ...
            getShare:function(){
                ...
                this.axios({
                    type: 'post',     // 通过设置type，来选择是get还是post请求
                    url: this.getAjax+'/user/share?pageSize='+this.pageSize+'&pageNum='+this.pageNum+'&openId='+this.openId,
                    // url: this.getAjax+'/newsRotatePic.do',    // 访问的后端接口地址
                    params: {                // get请求使用params,post请求使用data(data为json格式)
